...
- (BOOL)pollUntil:(int)milliseconds within:(NSArray *)contexts
{
    ...
    if (pollfds_capacity < i + 2) {
        ...
        if (pollfds == 0) {
#if	GS_WITH_GC
            pollfds = NSAllocateCollectable(pollfds_capacity * sizeof(*pollfds), 0);
#else
            pollfds = NSZoneMalloc(NSDefaultMallocZone(), pollfds_capacity * sizeof(*pollfds));
#endif
        } else {
#if	GS_WITH_GC
            pollfds = NSReallocateCollectable(pollfds, pollfds_capacity * sizeof(*pollfds), 0);
#else
            pollfds = NSZoneRealloc(NSDefaultMallocZone(), pollfds, pollfds_capacity * sizeof(*pollfds));
#endif
        }
    }
    pollfds_count = 0;
        ...
        if (info->_invalidated == YES) {
            ...
            switch (info->type) {
                ...
                case ET_RPORT: {
                    ...
                    NSDebugMLLog(@"NSRunLoop", @"listening to %"PRIdPTR" port handles\n", port_fd_count);
                    while (port_fd_count--) {
    ...
    while (completed == NO) {
        ...
        if (pollfds[fdIndex].revents != 0) {
            ...
            if (pollfds[fdIndex].revents & (POLLPRI|POLLERR|POLLHUP|POLLNVAL)) {
                watcher = (GSRunLoopWatcher *)NSMapGet(_efdMap, (void *)(intptr_t)fd);
            ...
            DLog(@"pollfds[fdIndex].revents: %d", pollfds[fdIndex].revents);
...
- (BOOL)pollUntil:(int)milliseconds within:(NSArray *)contexts
{
    ...
    //DLog(@"select timeout %d,%d", timeout.tv_sec, timeout.tv_usec);
    select_return = select(fdEnd, &read_fds, &write_fds, &exception_fds, select_timeout);
    
    NSDebugMLLog(@"NSRunLoop", @"select returned %d", select_return);
    //DLog(@"select returned %d", select_return);
    if (select_return < 0) {
        if (errno == EINTR) {
            GSPrivateCheckTasks();
            select_return = 0;
    ...
    while (completed == NO) {
        ...
        if (FD_ISSET (fdIndex, &exception_fds)) {
            GSRunLoopWatcher *watcher;
            watcher = (GSRunLoopWatcher*)NSMapGet(_efdMap, (void *)(intptr_t)fdIndex);
            if (watcher != nil && watcher->_invalidated == NO) {
                i = [contexts count];
                while (i-- > 0) {
                    GSRunLoopCtxt *c = [contexts objectAtIndex: i];
...
+ (BOOL)awakenedBefore:(NSDate *)when
{
    ...
    if (select(threadInfo->inputFd, &read_fds, &write_fds, &exception_fds, &timeout) > 0) {
        NSDebugMLLog(@"NSRunLoop", @"Fire perform on thread");
        DLog(@"Fire perform on thread");
        [threadInfo fire];
        return YES;
