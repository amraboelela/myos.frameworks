...
static FcPattern *opal_FcPatternCreateFromName(const char *name)
{
    ...
    family = strdup(name);
    //DLog(@"family: %s", family);
    pat = FcPatternCreate();
    //DLog(@"FcPatternCreate pat: %p", pat);
    if (!family || !pat) {
        goto error;
    }
    ...
    if (traits) {
        ...
        if (strstr(traits, "Italic")) {
            ...
            if (isCondensed) {
                if (isBold) {
                    ...
                    FcPatternAddString(pat, FC_FILE, (FcChar8 *)"/system/fonts/RobotoCondensed-Italic.ttf");
                }
            } else {
                if (isBold) {
                    FcPatternAddString(pat, FC_FILE, (FcChar8 *)"/system/fonts/Roboto-BoldItalic.ttf");
                } else {
                    FcPatternAddString(pat, FC_FILE, (FcChar8 *)"/system/fonts/Roboto-Italic.ttf");
                }
            }
            return pat;
#endif
        ...
        if (isCondensed) {
            if (isBold) {
                FcPatternAddString(pat, FC_FILE, (FcChar8 *)"/system/fonts/RobotoCondensed-Bold.ttf");
            } else {
                FcPatternAddString(pat, FC_FILE, (FcChar8 *)"/system/fonts/RobotoCondensed-Regular.ttf");
            }
        } else {
            if (isBold) {
                FcPatternAddString(pat, FC_FILE, (FcChar8 *)"/system/fonts/Roboto-Bold.ttf");
            }
        }
        if (!success) {
            DLog(@"!success 1");
...
FcPattern *opal_FcPatternCacheLookup(const char *name)
{
    ...
    {
        //DLog(@"Add new entry to the cache");
        cache_entry *entry;
