...
{
    return ((size.width == 0) && (size.height == 0)) ? YES : NO;
}
...
CGSize CGSizeCGRectUnion(CGSize size, CGRect rect)
{
    ...
    if (CGSizeIsZero(size)) {
        return CGSizeMake(rect.origin.x+rect.size.width,rect.origin.y+rect.size.height);
    } else if (CGRectIsEmpty(rect)) {
        return size;
    }
    result.width = MAX(size.width, rect.origin.x+rect.size.width);
    result.height = MAX(size.height, rect.origin.y+rect.size.height);
    return result;
}

CGSize CGSizeUnion(CGSize size1, CGSize size2)
{
...
{
    return [NSString stringWithFormat:@"x: %0.1f y: %0.1f width: %0.1f height: %0.1f", rect.origin.x, rect.origin.y, rect.size.width, rect.size.height];
}
