...
void CGContextFillEllipseInRect(CGContextRef ctx, CGRect rect)
{
    ...
    if (rect.size.width >= rect.size.height) {
        myRadius = rect.size.height / 2.0;
        CGContextAddArc(ctx, rect.origin.x + rect.size.width - myRadius, rect.origin.y + myRadius, myRadius, PI*1.5, PI/2.0 , NO);
        CGContextAddArc(ctx, rect.origin.x + myRadius, rect.origin.y + myRadius, myRadius, PI/2.0, PI*1.5, NO);
    } else {
        myRadius = rect.size.width / 2.0;
        CGContextAddArc(ctx, rect.origin.x + myRadius, rect.origin.y + rect.size.height - myRadius, myRadius, PI*1.5, PI/2.0 , NO);
        CGContextAddArc(ctx, rect.origin.x + myRadius, rect.origin.y + myRadius, myRadius, PI/2.0, PI*1.5, NO);
    }
    //CGContextAddRect(ctx, rect);
    CGContextClosePath(ctx);
