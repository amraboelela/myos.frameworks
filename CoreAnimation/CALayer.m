...
{
    CGRect rect;
    if (_modelLayer != self ||  CATransform3DIsIdentity(_transform)) {
        rect = CGRectMake(_position.x - _bounds.size.width * _anchorPoint.x,
                          _position.y - _bounds.size.height * (1 - _anchorPoint.y),
                          _bounds.size.width, _bounds.size.height);
    } else {
        //DLog(@"_transform: %@", CATransform3DDescription(_transform));
        CGSize boundsSize = _bounds.size;
        CGPoint rectCornerPoint1 = CGPointMake((1 - _anchorPoint.x) * boundsSize.width, (1 - _anchorPoint.y) * boundsSize.height);
        CGPoint transformPoint1 = CGPointMake(rectCornerPoint1.x * _transform.m11 + rectCornerPoint1.y * _transform.m21,
                                             rectCornerPoint1.x * _transform.m12 + rectCornerPoint1.y * _transform.m22);
        CGSize transformSize1 = CGSizeMake(fabs(transformPoint1.x / (1 - _anchorPoint.x)), fabs(transformPoint1.y / (1 - _anchorPoint.y)));
        
        CGPoint rectCornerPoint2 = CGPointMake((1 - _anchorPoint.x) * boundsSize.width, - _anchorPoint.y * boundsSize.height);
        CGPoint transformPoint2 = CGPointMake(rectCornerPoint2.x * _transform.m11 + rectCornerPoint2.y * _transform.m21,
                                             rectCornerPoint2.x * _transform.m12 + rectCornerPoint2.y * _transform.m22);
        CGSize transformSize2 = CGSizeMake(fabs(transformPoint2.x / (1 - _anchorPoint.x)), fabs(transformPoint2.y / _anchorPoint.y));
        
        CGSize transformSize = CGSizeUnion(transformSize1, transformSize2);//CGSizeMake((transformSize1.width + transformSize2.width) / 2.0, (transformSize1.height + transformSize2.height) / 2.0);
        
        rect = CGRectMake(_position.x - transformSize.width * _anchorPoint.x + _transform.m41,
                          _position.y - transformSize.height * (1 - _anchorPoint.y) + _transform.m42,
                          transformSize.width,
                          transformSize.height);
        //DLog(@"rect: %@", CGRectDescription(rect));
    }
    return rect;
}
...
{
    //DLog(@"transform: %@", CATransform3DDescription(transform));
...
{
    //DLog(@"sublayerTransform: %@", CATransform3DDescription(sublayerTransform));
...
{
    //DLog(@"affineTransform: %@", CGAffineTransformDescription(CATransform3DGetAffineTransform(_transform)));
...
{
    //DLog(@"self: %@", self);
    //DLog(@"affineTransform: %@", CGAffineTransformDescription(m));
...
- (CGPoint)convertPoint:(CGPoint)p toLayer:(CALayer *)l
{
    ...
    while (foreignGrandSuperLayer->_superlayer) {
        frame = foreignGrandSuperLayer.frame;
        lOrigin.x += frame.origin.x;
        lOrigin.y += frame.origin.y;
...
- (CGPoint)convertPoint:(CGPoint)p fromLayer:(CALayer *)l
{
    ...
    while (grandSuperLayer->_superlayer) {
        frame = grandSuperLayer.frame;
        localOrigin.x += frame.origin.x;
        localOrigin.y += frame.origin.y;
        grandSuperLayer = grandSuperLayer->_superlayer;
    }
    CALayer *foreignGrandSuperLayer = l;
    while (foreignGrandSuperLayer->_superlayer) {
...
{
    //DLog(@"_transform: %@", CATransform3DDescription(_transform));
    //DLog(@"self.frame: %@", CGRectDescription(self.frame));
